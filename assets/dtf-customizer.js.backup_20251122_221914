...
// Backend Configuration - Hostinger with SSL
const BACKEND_CONFIG = {
    local: 'https://api.designerts.online',
    render: 'https://api.designerts.online'
};
...
const state = {
    ...
    useLocalBackend: false, // Always use Hostinger backend
    colorAnalysisInProgress: false
};

// Expose state to window for cross-script access (needed for Shopify integration)
window.state = state;
...
// Store color pixels with progress updates
async function storeColorPixelsWithProgress(colors) {
    console.log('📦 storeColorPixelsWithProgress called with', colors.length, 'colors');
    ...
    for (let i = 0; i < colors.length; i++) {
        ...
        const colorHex = color.detected_hex || '#000000';
        const colorName = `${colorHex} (${i + 1}/${colors.length})`;

        console.log(`🎨 Storing pixels for color ${i + 1}/${colors.length}:`, color);

        // Update both loaders with detailed status
        updateLoaderStatus(`Storing pixels for color ${i + 1} of ${colors.length}`, `Color: ${colorHex}`);
        updateLoaderDetails(processed, 0, errors);
        updateColorExtractionAnimation(`Storing color ${i + 1} of ${colors.length}`, `Color: ${colorHex}`);

        try {
            const blob = await getImageBlob();
            if (!blob) {
                console.error('❌ getImageBlob returned null for color', i + 1);
                errors++;
                updateLoaderStatus(`Error: Failed to get image for color ${i + 1}`, '');
                updateLoaderDetails(processed, 0, errors);
                updateColorExtractionAnimation(`Error: Failed to get image for color ${i + 1}`, '');
            ...
            formData.append('color_id', color.uniqueId);
            formData.append('color_r', color.detected_rgb[0]);
            formData.append('color_g', color.detected_rgb[1]);
            formData.append('color_b', color.detected_rgb[2]);
            formData.append('tolerance', '10');

            console.log(`📤 Sending pixel storage request for ${color.uniqueId}`);
            ...
            console.log(`📥 Response for color ${i + 1}:`, response.status, response.ok);

            if (response.ok) {
                const result = await response.json();
                console.log(`✅ Pixels stored for color ${i + 1}:`, result);
                processed++;
                updateLoaderStatus(`Stored pixels for color ${i + 1} of ${colors.length}`, `✓ ${colorHex}`);
                updateLoaderDetails(processed, 0, errors);
                updateColorExtractionAnimation(`Stored color ${i + 1} of ${colors.length}`, `✓ ${colorHex}`);
            } else {
                const errorText = await response.text();
                console.error(`❌ Pixel storage failed for color ${i + 1}:`, errorText);
                errors++;
                updateLoaderStatus(`Failed to store color ${i + 1}`, `✗ ${colorHex}`);
                updateLoaderDetails(processed, 0, errors);
                updateColorExtractionAnimation(`Failed to store color ${i + 1}`, `✗ ${colorHex}`);
            }

            // Longer delay to make progress visible
            await new Promise(resolve => setTimeout(resolve, 400));

        } catch (error) {
            console.error(`❌ Error storing pixels for color ${i + 1}:`, error);
            errors++;
            updateLoaderStatus(`Error storing color ${i + 1}`, error.message);
            updateLoaderDetails(processed, 0, errors);
            updateColorExtractionAnimation(`Error storing color ${i + 1}`, error.message);
            await new Promise(resolve => setTimeout(resolve, 400));
        }
    }

    console.log(`📊 Pixel storage complete: ${processed} successful, ${errors} errors`);
}

// Helper function to get image blob
async function getImageBlob() {
    console.log('🖼️ getImageBlob called');

    // Priority 1: Use state.image (the original uploaded image)
    if (state.image) {
        console.log('✅ Using state.image');
    ...
    console.log('⚠️ state.image not found, checking preview...');

    // Priority 2: Check preview for canvas
    const preview = document.getElementById('dtf-preview');
    if (preview) {
        const previewCanvas = preview.querySelector('canvas');
        if (previewCanvas) {
            console.log('✅ Using canvas from preview');
            return new Promise(resolve => previewCanvas.toBlob(resolve, 'image/png'));
        }

        // Priority 3: Check preview for img
        const img = preview.querySelector('img');
        if (img) {
            console.log('✅ Using img from preview');
            const canvas = document.createElement('canvas');
    ...
    console.error('❌ No image source found for blob creation');
    return null;
}

// Color Analysis Loader Control Functions
function showColorAnalysisLoader() {
    ...
    if (loader) {
        ...
        state.colorAnalysisInProgress = true;
        ...
        updateLoaderStatus('Initializing color analysis...', '');
    }
}

function hideColorAnalysisLoader() {
    const loader = document.getElementById('dtf-color-loader');
    const placeholder = document.getElementById('dtf-preview-placeholder');

    if (loader) {
        loader.style.display = 'none';
        state.colorAnalysisInProgress = false;

        // Show placeholder if no image is loaded
        if (placeholder && !state.image) {
            placeholder.style.display = 'flex';
        }
    }
}

function updateLoaderStatus(status, details = '') {
...
function updateLoaderDetails(processed = 0, skipped = 0, errors = 0) {
    let detailsHtml = '';
...
function setupEventListeners() {
    ...
    // Preview interactions - now handled by canvas in updatePreview()
    // Event listeners are attached to the canvas element when it's created
}
...
async function extractColorsWithPalette(img) {
    try {
        ...
        // Show color analysis loader
        showColorAnalysisLoader();
        updateLoaderStatus('Preparing image for analysis...', '');

        showMessage('🔍 Analyzing image colors...', 'info');

        // Show loading animation in the color detection area with clear message
        showColorExtractionAnimation('Fetching colors from image...');
        updateColorExtractionAnimation('Fetching colors from image...', 'Please wait...');

        // Convert image to blob for upload
        updateLoaderStatus('Converting image for analysis...', '');
        ...
        updateLoaderStatus('Uploading image to backend...', '');
        updateColorExtractionAnimation('Uploading to server...', 'Analyzing colors...');
        ...
        updateLoaderStatus('Processing colors with AI...', '');
        updateColorExtractionAnimation('Processing with AI...', 'Detecting colors...');
        ...
        if (!response.ok) {
            ...
            updateLoaderStatus('Error: Backend processing failed', '');
            updateLoaderDetails(0, 0, 1);
            setTimeout(() => hideColorAnalysisLoader(), 2000);
            throw new Error(`HTTP error! status: ${response.status}, details: ${errorText}`);
        }

        updateLoaderStatus('Analyzing color results...', '');

        const result = await response.json();
        console.log('📥 Palette matching result:', result);

        // DON'T hide the animation yet - keep it visible for pixel storage
        // hideColorExtractionAnimation(); // Moved to after pixel storage completes
        ...
        if (result.success) {
            ...
            // Update animation to show we're moving to pixel storage phase
            updateColorExtractionAnimation(`Preparing to store ${merged.length} colors...`, 'Getting ready...');

            // Update loader with success status
            updateLoaderStatus(`Preparing to store pixels for ${merged.length} colors...`, '');
            updateLoaderDetails(0, reducedBy, 0);

            // Store pixels for each detected color with progress updates
            await storeColorPixelsWithProgress(state.colorMatches);

            // Final success update
            updateLoaderStatus(`✓ All ${merged.length} colors stored successfully!`, 'Ready to customize');
            updateLoaderDetails(merged.length, reducedBy, 0);
            updateColorExtractionAnimation(`✓ All ${merged.length} colors stored!`, 'Ready to customize');

            // Hide loader after showing success (longer delay to show completion)
            setTimeout(() => {
                hideColorAnalysisLoader();
        ...
        // Update loader with error status
        updateLoaderStatus('Backend unavailable, using fallback...', '');
        updateLoaderDetails(0, 0, 1);
...
function extractColorsFrontendFallback(img) {
    ...
    // Show loader for frontend fallback
    showColorAnalysisLoader();
    updateLoaderStatus('Using frontend color analysis...', '');

    // Simulate processing steps with progress updates
    updateLoaderStatus('Processing image pixels...', '');
    ...
    updateLoaderStatus('Analyzing color data...', '');
    ...
    // Process pixels with progress updates
    const stepSize = Math.max(1, Math.floor(data.length / 100)); // Update every 1%
    let processed = 0;
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        if (i % (stepSize * 4) === 0) {
            processed++;
            const progress = Math.round((processed / 100) * 100);
            updateLoaderStatus(`Analyzing colors... ${progress}%`, '');
            updateLoaderDetails(0, 0, 0);
        }
    }
    ...
    // Update loader with frontend fallback success
    updateLoaderStatus('Frontend analysis complete!', '');
    updateLoaderDetails(state.colorMatches.length, 0, 0);
...
function updatePreview() {
    ...
    // Set up mouse events on the canvas for dragging
    ...
    console.log('✅ Pan/drag event listeners attached to canvas');
}
...
// Mouse interaction functions for pan/drag
function startPan(e) {
...
function pan(e) {
    ...
    console.log('🖱️ Panning:', { x: state.pan.x, y: state.pan.y });
...
function showColorExtractionAnimation(message = 'Analyzing colors...') {
...
function updateColorExtractionAnimation(message, progress = '') {
    const animationContainer = document.querySelector('.color-extraction-animation');
    if (!animationContainer) return;

    const textElement = animationContainer.querySelector('.animation-text');
    const progressElement = animationContainer.querySelector('.animation-progress');
...
function zoomImage(factor) {
    ...
    // Smooth zoom - smaller increments for better control
    const targetZoom = state.zoom * factor;
    state.zoom = Math.max(0.5, Math.min(3, targetZoom)); // Limit zoom between 50% and 300%
...
function fitImageToScreen() {
    ...
    // Reset zoom to 1 and center pan to 0
    state.zoom = 1;
    state.pan = { x: 0, y: 0 };

    console.log('📐 Reset to fit: zoom=1, pan={0,0}');
...
// Make zoom functions globally available for Shopify integration
window.zoomImage = zoomImage;
window.fitImageToScreen = fitImageToScreen;
...
// Expose functions globally for Shopify integration
window.saveColorChange = saveColorChange;
window.closeColorPopup = closeColorPopup;
window.openColorPopup = openColorPopup;
